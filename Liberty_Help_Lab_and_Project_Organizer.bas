...
cursor hourglass
 ...
 global addfastfunction, selectedKey$, funcs$, project, fnamenobas$, DestPath$, DestPath1$, FolderDialog$, pnum, fastfuncs$, JBexe$, JBpath$, spritecreated, jbReservedWords$, dictionary$, keyCount, q$, fileToCheck$, lastKey$, helpFilePath$, resetsearch, closehtml, categorie$, upath$, selectedpath$
  ...
  JBpath$ = "c:\Program Files (x86)\Liberty BASIC v4.5.1"
JBexe$ = "liberty.exe"
JBruntime$ = "run451.exe"
  tutorialPath$ = JBpath$;"\jbtutorial\index.html"
  openhelp$ = JBpath$;"\lb4help\LibertyBASIC_4_web\amber_menu.htm"
  helpFilePath$ = JBpath$;"\lb4help\LibertyBASIC_4_web"
...
'top menu
    ...
    menu #main, "Edit"
    menu #main, "View", "This Help Lab's Progress Log", [labLog], "Liberty Basic Error Log", [jberrorLog], "Runtime Error Log", [runtimeLog], "Help Lab Error Log", [helplaberrorLog]
...
'jbsearch by cundo
    ...
    button #main.tutorial, "Open LB Tutoria&l", [opentutorial], UL, 523, 365, 115, 20
...
'buttons bottom left and middle
   ...
   button #main.runjb, "Edit Selected with LB &IDE", [edit_In_JB_IDE], ul, 330, 652, 180, 20
   button #main.editInNotepad, "Edit Selected with Notep&ad", [editInNotepad], ul, 580, 652, 180, 20
'create buttons
   button #main.jbProgs, "Liberty &Basic", [jbProgs], ul, 1085, 30, 120, 25
   ...
   button #main.task, " Task Ma&nager ", [taskman], ul, 1085, 125, 120, 23
   ...
   button #main.jberrorlog, "LB Error Log", [jberrorLog], ul, 1085, 340, 85, 23
...
'jb samples  to reserved words comboboxes and statictext
   statictext  #main.jbsamplestext, "LB Examples", 1140, 420, 125, 15
   combobox #main.jbsamplesList, jbsamplesList$(), jbsampleSelected , 1120, 435, 120, 25
   statictext  #main.functionstext, "LB Functions", 1247, 420, 100, 15
   combobox #main.jbfunctionsList, jbfunctionsList$(), jbfunctionSelected , 1240, 435, 100, 25
   statictext  #main.jbdialogstext, "LB Dialogs", 1255, 465, 100, 15
...
'jb bak(up) Files combobox and statictext
   statictext  #main.jbbaktext, "LB BAK (UP) Files", 1095, 375, 120, 15
...
'right side static text
   ...
   statictext  #main.jbforums, "Visit https://libertybasiccom.proboards.com/", 05, 700, 275, 25
'kill all button
...
jbusppath$ = upath$+"\AppData\Roaming\Liberty Basic v4.5.1\SPRITES"
run "explorer.exe ";q$;jbusppath$;q$
else
if pathExists(upath$+"\Application Data\Liberty Basic v4.5.1\SPRITES") <> 0 then
jbusppath$ = upath$+"\Application Data\Liberty Basic v4.5.1\SPRITES"
run "explorer.exe ";q$;jbusppath$;q$
end if
end if
wait
...
jbusppath$ = upath$+"\AppData\Roaming\Liberty Basic v4.5.1\bmp"
run "explorer.exe ";q$;jbusppath$;q$
else
if pathExists(upath$+"\Application Data\Liberty Basic v4.5.1\bmp") <> 0 then
jbusppath$ = upath$+"\Application Data\Liberty Basic v4.5.1\bmp"
run "explorer.exe ";q$;jbusppath$;q$
end if
end if
wait
...
jbusppath$ = upath$+"\AppData\Roaming\Liberty Basic v4.5.1"
run "explorer.exe ";q$;jbusppath$;q$
else
if pathExists(upath$+"\Application Data\Liberty Basic v4.5.1") <> 0 then
jbusppath$ = upath$+"\Application Data\Liberty Basic v4.5.1"
run "explorer.exe ";q$;jbusppath$;q$
end if
end if
wait
...
chr$(13);_
chr$(13);_
"    Credit goes to cundo for jbsearch(the LB help file search engine)";_
...
#lablog "@- [jberrorLog]opening the jb error log ............"
       if fileExists(upath$;"\AppData\Roaming\Liberty Basic v4.5.1", jberrorlog$) then
run "notepad ";q$;upath$;"\AppData\Roaming\Liberty Basic v4.5.1\error.log";q$
else
      if fileExists(upath$;"\Application Data\Liberty Basic v4.5.1", jberrorlog$) then
...
#lablog "entering sub jbsampleSelected jbsamplesList$..........."
  q$ = chr$(34)
  call saveValue
    print "jbSamplesPath = ";upath$;"\AppData\Roaming\Liberty Basic v4.5.1\";jbsamps$
    #main.jbsamplesList, "selection? jbsamps$"
    jbsamps$ = jbsamps$;".bas"
    runFile$ = upath$;"\AppData\Roaming\Liberty Basic v4.5.1\";jbsamps$
    print "jbSamplesPath = ";upath$;"\AppData\Roaming\Liberty Basic v4.5.1\";jbsamps$
    jbRunIt$ = JBpath$;"\";JBexe$
  if fileExists(upath$;"\AppData\Roaming\Liberty Basic v4.5.1", jbsamps$) <> 0 then
    runFile$ = upath$;"\Application Data\Liberty Basic v4.5.1\";jbsamps$
    run q$;jbRunIt$;q$;" ";q$;runFile$;q$
    else
  if fileExists(upath$;"\Application Data\Liberty Basic v4.5.1", jbsamps$) <> 0 then
    runFile$ = upath$;"\Application Data\Liberty Basic v4.5.1\";jbsamps$
    run q$;jbRunIt$;q$;" ";q$;runFile$;q$
    else
...
#main.jbdialogsList, "selection? jbdialog$"
    if jbdialog$ = " Color Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe00mf.htm"
    if jbdialog$ = " Printer Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe72sp.htm"
    if jbdialog$ = " File Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe7ezo.htm"
    if jbdialog$ = " All JBDialogs" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe6gmr.htm"
    if jbdialog$ = " Font Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe7sc5.htm"
    if jbdialog$ = " Confirm Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe4pps.htm"
    if jbdialog$ = " Notice Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe1836.htm"
   if jbdialog$ = " Prompt Dialog" then runFile$ = JBpath$;"\lb4help\LibertyBASIC_4_web\html\libe2p4c.htm"
...
q$ = chr$(34)
  call saveValue
    #main.jbbakfilesList, "selection? jbbakfile$"
      ...
      print "jbbakfilePath = ";upath$;"\AppData\Roaming\Liberty BASIC v4.5.1\bak";jbbakfile$ 
 if fileExists(upath$;"\AppData\Roaming\Liberty BASIC v4.5.1\bak", jbbakfile$) <> 0 then
    runFile$ = upath$;"\AppData\Roaming\Liberty BASIC v4.5.1\bak\";jbbakfile$
    jbRunIt$ = JBpath$;"\";JBexe$
    run q$;jbRunIt$;q$;" ";q$;runFile$;q$
    else
 if fileExists(upath$;"\Application Data\Liberty BASIC v4.5.1\bak\", jbbakfile$) <> 0 then
    runFile$ = upath$;"\Application Data\Liberty BASIC v4.5.1\bak\";jbbakfile$
    jbRunIt$ = JBpath$;"\";JBexe$
    run q$;jbRunIt$;q$;" ";q$;runFile$;q$
    else
...
jbFontDialog$ = " All JBDialogs, Prompt Dialog, Notice Dialog, Font Dialog, Color Dialog, File Dialog, Printer Dialog, Confirm Dialog"
 ...
 #lablog," entering sub getjbreservedwords "
  ...
  next x
     sort jbreservedwordsList$(), 1 ,300
...
q$ = chr$(34)
 ...
 [skip]
  Next x
     sort jbbakfilesList$(), 0 ,numberOfFiles
...
if fileExists(savedProjects$;"\";selectedKey$,selectedKey$;".bas") = 0 then notice "Project wasn't saved"+chr$(13)+"Try [Make New Project]"+chr$(13)+"And Select the appropriate .bas file" : wait
goto [bas2exe]
...
'if Liberty Basic v4.5.1 is NOT installed to it's Default Install Dir, get Path from User using folder dialog
    if res then [go] else notice chr$(13)+" Liberty Basic v4.5.1 was not installed to the default install folder." +chr$(13)+"Hit [ok], then Select the Folder iberty Basic v4.5.1 is Installed"
...
print "lb install path = ";FolderDialog$
#lablog,"Got lb install path using VBS FolderDialog Script = ";FolderDialog$
...
'add some text ,some buttons, and checkboxes to the Window
    ...
    statictext #pick.jbforums, "Visit the Liberty Basic Forums @ https://libertybasiccom.proboards.com/", 90, 335, 590, 20
...
'check existence and JBPath$ (Liberty Basic default install dir)
#lablog, "checking path existence for ";JBpath$
res = pathExists(JBpath$)
  if res then a = a + 1 else notice " Liberty Basic v4.5.1 was not was not found in ";JBpath$;" aborting BAS2EXE":wait
...
#lablog, "Checking JBexe$"
res=fileExists(JBpath$, JBexe$)
   if res then a = a + 1 else notice JBexe$;" JBexe$ Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking JBruntime$"
res=fileExists(JBpath$,JBruntime$)
    if res then a = a + 1 else notice JBrun$;" JBrun$ Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vbas31w.sll"
res=fileExists(JBpath$,"vbas31w.sll")
    if res then a = a + 1 else notice " vbas31w.sll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vgui31w.sll"
res=fileExists(JBpath$,"vgui31w.sll")
    if res then a = a + 1 else notice " vgui31w.sll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking voflr31w.sll"
res=fileExists(JBpath$,"voflr31w.sll")
    if res then a = a + 1 else notice " voflr31w.sll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vthk31w.dll"
res=fileExists(JBpath$,"vthk31w.dll")
    if res then a = a + 1 else notice " vthk31w.dll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vtk1631w.dll"
res=fileExists(JBpath$,"vtk1631w.dll")
    if res then a = a + 1 else notice " vtk1631w.dll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vtk3231w.dll"
res=fileExists(JBpath$,"vtk3231w.dll")
    if res then a = a + 1 else notice " vtk3231w.dll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vvm31w.dll"
res=fileExists(JBpath$,"vvm31w.dll")
    if res then a = a + 1 else notice " vvm31w.dll Does not exist in  ";JBpath$;" aborting BAS2EXE" : wait
#lablog, "Checking vvmt31w.dll"
res=fileExists(JBpath$,"vvmt31w.dll")
    if res then a = a + 1 else notice " vvmt31w.dll Does not exist in  ";JBpath$;" aborting BAS2EXE":wait
...
'removing any spaces from filename, replacing with an   _  character
 dim s$(250)
